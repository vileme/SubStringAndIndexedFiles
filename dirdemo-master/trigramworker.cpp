...
TrigramWorker::TrigramWorker(QObject *parent,qint64 size) : QObject(parent),size(size) {}
...
void TrigramWorker::process_files() {
    ...
    for (auto i: files) {
        process_file(i);
        qint64 fileSize = QFile(i.second).size();
        curr+=fileSize;
        double updateSize = double(curr*100)/size;
